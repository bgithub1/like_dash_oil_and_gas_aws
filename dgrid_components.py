...
def flatten_layout(app):
    ...
    #     pdb.set_trace()
        if  hasattr(c,'children') and 'div.div' in str(type(c)).lower() and len(c.children)>0 and type(c.children) is not list:
...
#                 o = Output(self.cid,p)
        ...
        if className is not None:
            self.div = html.Div([self.component],className=className)
        else:
...
class DivComponent(ComponentWrapper):
    def __init__(self,component_id,input_component=None,
                 ...
                 callback_input_transformer=None,
                 style=None,className=None,logger=None):
...
class DropdownComponent(ComponentWrapper):
    def __init__(self,component_id,
        ...
        super(DropdownComponent,self).__init__(radio,
             input__tuples=input_tuples,
             output_tuples=[(component_id,'value')],
             style=style,
             className=className,
             logger=logger,
...
class MarkdownComponent(ComponentWrapper):
    def __init__(self,component_id,
        ...
        super(MarkdownComponent,self).__init__(
            ...
            style=markdown_style,className=className),logger=logger)
...
class UploadFileNameDiv(DivComponent):
    def __init__(self,component_id,upload_component,text=None,style=None,
                 className=None,initial_file_to_show=None):
...
class DashTableComponent(ComponentWrapper):
    def __init__(self,component_id,df_initial,input_component=None,title=None,
                 transform_input=None,editable_columns=None,style=None,
                 className=None,logger=None,
...
class XyGraphComponent(ComponentWrapper):
    def __init__(self,component_id,input_component,x_column,
        ...
        # define dash_table callback using closure so that you don't refer back to 
        #   class instance during the callback
        def _create_gr_lambda(component_id,x_column,plot_title,logger,transform_input=None,num_x_values=None):
                    ...
                    if value_list is not None and len(value_list)>0 and value_list[0] is not None:
                        ...
                        if df is not None:
                            fig = plotly_plot(df,x_column,
                                    plot_title=plot_title,bar_plot=plot_bars,marker_color=marker_color,number_of_ticks_display=num_x_values)
                            ret =  [fig]
                except Exception as e:
                    traceback.print_exc()
        ...
        # define callback so that it includes self.logger
        gr_lam = _create_gr_lambda(self.component_id,x_column,t,self.logger,transform_input=transform_input,num_x_values=num_x_values)
...
class FigureComponent(ComponentWrapper):
    def __init__(self,component_id,
                 ...
                 input_component_property='data',
                 input_tuple_list=None,
                 figure=None,style=None,className=None,logger=None):
        ...
        # do super, but WITHOUT the callback
        itl = [(input_component.id,input_component_property)] if input_tuple_list is None else input_tuple_list
        super(FigureComponent,self).__init__(gr_html,
                     input__tuples=itl,#[(input_component.id,input_component_property)],
                     output_tuples=[(self.component_id,'figure')],
                     className=className,
                     callback_input_transformer=lambda v:[None])
...
class StoreComponent(ComponentWrapper):        
    def __init__(self,component_id,
                 input_component_list=None,
                 create_storage_dictionary_from_inputs_callback=None,
                 initial_data = None,
                 logger=None):
        ...
        # do super
        cb = lambda v: [v]
        if create_storage_dictionary_from_inputs_callback is not None:
            cb = create_storage_dictionary_from_inputs_callback
        super(StoreComponent,self).__init__(dcc_store,
                     input__tuples=None if input_component_list is None else input_component_list,
                     output_tuples=[(component_id,'data')],
                     callback_input_transformer = None,
                     style={'display':'none'},
                     logger=logger)
        
        self.callback_input_transformer  = _create_callback_lambda(component_id,cb, self.logger) 
...
def recursive_nested_layout(grid_tuple,layout_components,callback_components,style=None):
    '''

    top = (top,'100','100')
    mid_left = ([t1, t2, t3], '100','33 33 33')
    mid_right = ([c1...c9],'33 33 33','33 33 33')
    mid = ([mid_left,mid_right],'20 80','100')
    bottom = (bottom,'100','100')
    
    main = ([top,mid,bottom],'100','100')
    layout_components = []
    callback_components = []
    app = recursive_nested_layout(main,layout_components,callback_components)
    
    :param grid_tuple:
    :param layout_components:
    :param callback_components:
    :param style
    '''
    
    # get components    
    components_or_tuples = grid_tuple[0]
    # loop through rows, then columns
    grid_template_columns = grid_tuple[1]
    grid_template_rows = grid_tuple[2]
    s = {} if style is None else style
    s['grid-template-columns'] = grid_template_columns
    s['grid-template-rows'] = grid_template_rows
    gtr_len = len(grid_template_rows.split(" "))
    gtc_len = len(grid_template_columns.split(" "))
    component_array = []
    for r in range(gtr_len):
        for c in range(gtc_len):
            this_component = components_or_tuples[r*gtc_len + c]
            if type(this_component) is not tuple:
                component_html = this_component.html if hasattr(this_component, 'html') else this_component
                this_html = html.Div([component_html],style=STYLE_MIDDLE_CENTER)
                component_array.append(this_html)
                if isinstance(this_component, ComponentWrapper):
                    callback_components.append(this_component)                
            else:
                recursive_nested_layout(this_component,layout_components,callback_components,s)
    this_grid = make_2d_grid(component_array,style=s)
    layout_components.append(this_grid)
    
    

def make_grid_app(top_level_grid_tuple,style=None):
    callback_components = []
    layout_components = []
    s = DEFAULT_GRID_STYLE if style is None else style
    # populate layout_components using recursive algo
    recursive_nested_layout(top_level_grid_tuple,layout_components,callback_components,style=s)
    ret_app =   dash.Dash() 
    ret_app.layout = html.Div(layout_components,style={'margin-left':'10px','margin-right':'10px'})

    for ac in callback_components:
        if hasattr(ac, 'route'):
            ac.route(ret_app)
    [c.callback(ret_app) for c in callback_components]
    return ret_app
    

def make_2d_grid(component_array,style=None,
                 wrap_in_loading_state=False):
    # create style for grid, and then add in another style elements pass in by the style arg
    gs = DEFAULT_GRID_STYLE if style is None else style
